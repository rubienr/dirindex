...
class ConfigFileHelper:
    ...
    def read_config_file(self):
        ...
        if not config.has_option("DEFAULT", "database_file_path"):
            print(
                "WARNING: 'database_file_path' - not set in the loaded configuration. Using default path.")
        self._database_file_path = config.get("DEFAULT", "database_file_path")
        if self._database_file_path is None:
            self._database_file_path = os.getcwd() + "/{}".format(DEFAULT_DB_NAME)

        if not os.path.isdir(self._database_file_path[0:self._database_file_path.rfind('/')]):
            print("WARNING: 'database_file_path' - Loaded path does not exist. Using default path.")
            self._database_file_path = os.getcwd() + "/{}".format(DEFAULT_DB_NAME)

        if not os.path.isfile(self._database_file_path):
            Path(self._database_file_path).touch()

        if not config.has_section("paths"):
            raise ValueError("ERROR: '[paths]' section not present in the loaded configuration")
        if not config.has_option("paths", "folders"):
            raise ValueError("ERROR: '[paths]' section does not contain 'folders' parameter")
